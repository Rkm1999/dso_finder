...
export default {
  ...
  computed: {
    filteredTargetList() {
      ...
      return this.completeTargetList.filter(target => {
        const matchesQuery = target.name.toLowerCase().includes(query) ||
                             target.ra.toLowerCase().includes(query) ||
                             target.dec.toLowerCase().includes(query);
        return matchesQuery; // Commented out altitude check
        // return matchesQuery && target.altitude > 0;
      });
    },
    completeTargetList() {
  ...
  methods: {
    ...
    //     const [azimuth, altitude] = this.raDecToAltAz(
